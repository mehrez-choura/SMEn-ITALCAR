  ...
  const SITES_DATA = {
    MEGRINE: {
      ...
      elecUsage: [
        ...
        { name: "Gaz (Four)", value: 3, kpi: "kWh/Véhicule", significant: false, note: "Non-Significatif (<5%)" }
      ]
    },
    ELKHADHRA: {
      name: "El Khadhra",
       ...
       <header className="bg-gradient-to-r from-emerald-700 to-teal-800 text-white shadow-lg sticky top-0 z-30">
            <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    <button 
                        ...
                        className="flex items-center bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-lg border border-white/20 text-xs font-bold transition-colors"
                    >
        ...
        {showHistoryInput && (
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowHistoryInput(false)}>
                <div className="bg-white rounded-xl max-w-lg w-full p-6 shadow-2xl relative" onClick={e => e.stopPropagation()}>
                    <button onClick={() => setShowHistoryInput(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600">✕</button>
                    <h3 className="font-bold text-slate-800 mb-4 flex items-center text-lg"><Database className="mr-2 text-emerald-600" /> Historique Consommation</h3>
                    <p className="text-sm text-slate-600 mb-4">Fonctionnalité prête pour l'intégration de vos données historiques (2018-2024).</p>
                    <div className="bg-slate-100 p-4 rounded text-center text-slate-500 text-sm border border-slate-200 border-dashed">
                        Interface de saisie grille (Mois x Année) à venir...
                    </div>
                </div>
            </div>
        )}
        ...
        <main className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="flex space-x-2 mb-8 overflow-x-auto pb-2">
                {Object.keys(SITES_DATA).map(key => (
                    <button
                        ...
                        className={`px-6 py-3 rounded-xl font-bold text-sm transition-all whitespace-nowrap flex items-center
                            ${activeSiteTab === key 
            ...
            {/* KEY POINTS (Points Clés en Haut) */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center">
                    <div className="p-3 rounded-full bg-blue-50 text-blue-600 mr-4"><Maximize2 size={20} /></div>
                    <div>
                        <div className="text-xs text-slate-500 uppercase font-bold">Surface Totale</div>
                        <div className="text-xl font-black text-slate-800">{currentData.area.toLocaleString()} m²</div>
                    </div>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center">
                    <div className="p-3 rounded-full bg-emerald-50 text-emerald-600 mr-4"><Zap size={20} /></div>
                    <div>
                        <div className="text-xs text-slate-500 uppercase font-bold">Source Principale</div>
                        <div className="text-xl font-black text-slate-800">{currentData.energyMix[0].name}</div>
                    </div>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center">
                    <div className="p-3 rounded-full bg-amber-50 text-amber-600 mr-4"><TrendingUp size={20} /></div>
                    <div>
                        <div className="text-xs text-slate-500 uppercase font-bold">Poste #1</div>
                        <div className="text-xl font-black text-slate-800">{currentData.elecUsage[0].name}</div>
                    </div>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center">
                    <div className="p-3 rounded-full bg-purple-50 text-purple-600 mr-4"><Leaf size={20} /></div>
                    <div>
                        <div className="text-xs text-slate-500 uppercase font-bold">Ratio Global</div>
                        <div className="text-xl font-black text-slate-800">-- <span className="text-xs font-normal text-slate-400">kWh/m²</span></div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                                    <div className="flex items-center text-xs text-slate-500 mb-1 font-bold">COUVERT</div>
                                    <div className="font-bold text-slate-700 text-lg">{currentData.covered.toLocaleString()} m²</div>
                                </div>
                                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                                    <div className="flex items-center text-xs text-slate-500 mb-1 font-bold">OUVERT</div>
                                    <div className="font-bold text-slate-700 text-lg">{currentData.open.toLocaleString()} m²</div>
                                </div>
                            </div>
                            <div className="text-xs text-slate-600 bg-blue-50 p-3 rounded-lg border border-blue-100 leading-relaxed">
                                <span className="font-bold text-blue-800 block mb-1 uppercase text-[10px]">Affectation des Zones :</span>
                                {currentData.details}
                            </div>
                        </div>
                    </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                        <h3 className="text-sm font-bold text-slate-400 uppercase mb-6 flex items-center justify-between">
                            <span className="flex items-center"><PieChart size={16} className="mr-2" /> Répartition Consommation</span>
                            <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-1 rounded">Basé sur audit énergétique</span>
                        </h3>
                            ...
                            {currentData.elecUsage.map((usage, idx) => (
                                <div key={idx} className="flex items-center justify-between group">
                                    <div className="w-1/3 pr-2">
                                        <div className="flex items-center">
                                            {usage.significant && <div className="w-2 h-2 rounded-full bg-red-500 mr-2 flex-shrink-0" title="Usage Significatif"></div>}
                                            <span className={`text-sm ${usage.significant ? 'font-bold text-slate-800' : 'text-slate-500'}`}>{usage.name}</span>
                                        </div>
                                        {usage.note && <div className="text-[9px] text-orange-500 ml-4 mt-0.5">{usage.note}</div>}
                                    </div>
                                    
                                    <div className="flex-1 px-4">
                                        <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
                                            <div 
                                                className={`h-full rounded-full ${usage.name.includes("Gaz") ? 'bg-orange-400' : (usage.significant ? 'bg-emerald-500' : 'bg-slate-300')}`} 
                                                style={{ width: `${usage.value}%` }}
                                            ></div>
                                        </div>
                                    </div>

                                    <div className="w-24 flex justify-end items-center text-right">
                                        <span className="font-bold text-slate-700 mr-2">{usage.value}%</span>
                                        <span className="text-[9px] text-slate-400 min-w-[50px] text-right">{usage.kpi}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                        
                        <div className="mt-6 flex items-center justify-end text-xs text-slate-400 gap-4 border-t border-slate-100 pt-3">
                            <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-emerald-500 mr-2"></span> Usage Significatif</span>
                            <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-slate-300 mr-2"></span> Usage Secondaire</span>
                            <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-orange-400 mr-2"></span> Gaz</span>
                        </div>
                    </div>

                    {/* Comparaison & Historique (Placeholder pour futures données) */}
                    <div className="bg-slate-100 rounded-2xl border-2 border-dashed border-slate-300 p-8 text-center">
                        <TrendingUp size={48} className="mx-auto text-slate-300 mb-4" />
                        <h3 className="text-slate-500 font-bold mb-2">Analyse Temporelle & Historique</h3>
                        <p className="text-sm text-slate-400 max-w-md mx-auto mb-4">
                            Cette section affichera les graphiques d'évolution mensuelle et la comparaison avec l'année de référence une fois les données historiques saisies.
                        </p>
                        <button onClick={() => setShowHistoryInput(true)} className="bg-white border border-slate-300 text-slate-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-white hover:text-emerald-600 hover:border-emerald-300 transition-colors">
                            Commencer la saisie
                        </button>
                    </div>
                </div>
...
// ... (Le reste du code MainDashboard et App reste inchangé) ...
